...
//
//
// Related Topics 数组 哈希表 滑动窗口 👍 631 👎 0

package leetcode.editor.cn;

//java:水果成篮
class P_904_FruitIntoBaskets {
    public static void main(String[] args) {
        Solution solution = new P_904_FruitIntoBaskets().new Solution();
    }

    //leetcode submit region begin(Prohibit modification and deletion)
    class Solution {
        public int totalFruit(int[] fruits) {
            // 滑动窗口模拟题：使用 left 和 right 分别代表滑动窗口的两端，窗口种类不超过 2 种为合法
            int left = 0;
            int right = 0;
            int category = 0;
            // 记录每种水果出现的次数
