...
class P_138_CopyListWithRandomPointer {
    public static void main(String[] args) {
        Solution solution = new P_138_CopyListWithRandomPointer().new Solution();
    }
...
*/
    ...
    class Solution {
        public Node copyRandomList00(Node head) {
            ...
            // 数据复制核心解法：一个哈希，两次遍历
            Node cur = head;
            // 建立旧节点对新节点的映射，便于后续针对next及random指针赋值
            Map<Node, Node> oldToNewMap = new HashMap<>();
            while (cur != null) {
                Node node = new Node(cur.val);
                oldToNewMap.put(cur, node);
                cur = cur.next;
            }

            cur = head;
            while (cur != null) {
                Node node = oldToNewMap.get(cur);
